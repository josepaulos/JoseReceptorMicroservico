        ...
        stage('Verificar Repositório') {
            steps {
                checkout([$class: 'GitSCM', branches: [[name: '*/main']],
                                          userRemoteConfigs: [[url: 'https://github.com/josepaulos/JoseReceptorMicroservico.git']]])
            }
        }
            ...
            steps {
                script {
                    withSonarQubeEnv('SonarQubeServer') {  // 'SonarQubeServer' é o nome da instância configurada no Jenkins
                        bat 'mvn clean verify sonar:sonar -Dsonar.projectKey=projeto -Dsonar.projectName=JoseReceptorMicroservico'
                    }
                }
            }
        }
